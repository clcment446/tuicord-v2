...
summary: The keyboard-workflow branch needs the full picker retained, its Vim Shell focus implementation restored, and its orphaned autobot integration excluded.
...
input-mode test without the Shell implementation, removed the picker key from
configuration, and imported an uncommitted `user-scripts/autobot` package.
...
restores the `ctrl+e` configuration/help entry, implements Shell's post-routing
focus requests for Vim `I` and `;q`, and removes only the orphaned autobot code.
File uploads accept empty text when files are present, while whitespace-only
